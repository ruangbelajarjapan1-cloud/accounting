...
async function loadStudents(){
  ...
  const h = [`<table class="min-w-full text-sm"><thead><tr class="text-left">
    ...
    ...rows.map(r=>`<tr class="border-t">
      ...
      <td class="p-2">${r.family_key||''}</td>
      <td class="p-2">${r.parent_name||''}</td>
      <td class="p-2">${r.parent_phone||''}<br>${r.email||''}</td>
...
function openPaymentForm(){
  ...
  const class_id   = prompt('ID Kelas (boleh kosong untuk auto semua kelas)','');
...
async function generateInvoices(){
  ...
  alert('Invoice dibuat: '+r.count+' file');
  loadInvoices();
}
